...
fn decode_len_check(bencode: &[u8]) -> Option<Element> {
    let mut len = 0;
...
/// # Arguments
/// * `bencode` - bencoded data **without** spaces.
fn decode_no_check(bencode: &[u8]) -> Option<Element> {
    let mut len = 0;
...
fn decode_with_spaces_len_check(bencode: &[u8]) -> Option<Element> {
    let without_spaces = erase_whitespaces(bencode);
...
/// # Arguments
/// * `bencode` - bencoded data.
fn decode_with_spaces_no_len_check(bencode: &[u8]) -> Option<Element> {
    let without_spaces = erase_whitespaces(bencode);
